...
class TestApiDirect:
    ...
    def test_should_direct_list_api(self):
        setup = _api_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-api", "live" if setup["live"] else "unit")
        if _skip:
