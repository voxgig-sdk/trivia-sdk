...
class TriviaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TriviaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TriviaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TriviaTestRunner.getenv("TRIVIA_TEST_LIVE")
        override = TriviaTestRunner.getenv("TRIVIA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TriviaTestRunner.getenv(key)
        ...
        explain = TriviaTestRunner.getenv("TRIVIA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TRIVIA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TriviaTestRunner._test_control is not None:
            return TriviaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TriviaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TriviaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TriviaTestRunner.load_test_control()
...
def load_env_local():
    TriviaTestRunner.load_env_local()


def env_override(m):
    return TriviaTestRunner.env_override(m)


def entity_data(v):
    return TriviaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TriviaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TriviaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TriviaTestRunner.load_test_control()


def live_client_options():
    return TriviaTestRunner.live_client_options()


def live_delay_ms():
    return TriviaTestRunner.live_delay_ms()
