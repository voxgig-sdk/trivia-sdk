...
//
// NewTriviaSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TriviaSDK {
	res, _ := kase["res"].([]any)
...
func fcCandidates(client *sdk.TriviaSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TriviaSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TriviaSDK, name string) any {
