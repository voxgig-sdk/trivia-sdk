...
{
    private static function utility(): TriviaUtility
    {
        return new TriviaUtility();
    }
    ...
    private static function ctx(array $over = []): TriviaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TriviaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TriviaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TriviaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TriviaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TriviaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TriviaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TriviaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TriviaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TriviaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TriviaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TriviaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TriviaResponse([]),
            'result' => new TriviaResult([]),
        ]);
        [, $err] = TriviaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TriviaSpec(['step' => 's']),
            'response' => new TriviaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TriviaResult([]),
        ]);
        [, $err] = TriviaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TriviaSpec(['step' => 's']),
            'response' => new TriviaResponse(self::resp(200, ['v' => 1])),
            'result' => new TriviaResult([]),
        ]);
        [, $err] = TriviaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TriviaSpec(['step' => 's']),
            'response' => new TriviaResponse(self::resp(200, ['v' => 2])),
            'result' => new TriviaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TriviaMakeResponse::call($ctx);
    ...
    {
        $preset = new TriviaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TriviaSpec([]),
            'response' => new TriviaResponse([]),
            'result' => new TriviaResult([]),
        ...
        [$got, $err] = TriviaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TriviaResult([])]);
        [, $err] = TriviaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TriviaSpec([]), 'result' => null]);
        [, $err] = TriviaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TriviaSpec(['step' => 's']),
            'result' => new TriviaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TriviaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TriviaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TriviaSpec(['step' => 's']),
            'result' => new TriviaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TriviaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TriviaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TriviaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TriviaSpec([]),
            'result' => new TriviaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TriviaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TriviaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TriviaError('boom', 'boom');
        $utility->fetcher = function (TriviaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TriviaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TriviaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TriviaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TriviaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TriviaResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TriviaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TriviaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TriviaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TriviaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TriviaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TriviaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TriviaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TriviaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TriviaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TriviaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TriviaResult(['ok' => false])]);
        $this->expectException(TriviaError::class);
        TriviaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TriviaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TriviaMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TriviaBaseFeature();
        $b = new TriviaBaseFeature();
        TriviaFeatureAdd::call($ctx, $a);
        TriviaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TriviaBaseFeature
    {
        $f = new TriviaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TriviaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TriviaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TriviaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TriviaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TriviaResponse(['headers' => null]),
            'result' => new TriviaResult([]),
        ]);
        TriviaResultHeaders::call($ctx);
