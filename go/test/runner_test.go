...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TRIVIA_TEST_LIVE") == "TRUE" ||
		os.Getenv("TRIVIA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TRIVIA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TriviaSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TriviaError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TriviaSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TriviaError{Msg: msg, Code: code}
}
