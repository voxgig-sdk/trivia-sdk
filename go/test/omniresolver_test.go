...
import (
	...
	omni "github.com/voxgig-sdk/trivia-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TriviaError); is {
				out := map[string]any{
					"name":    "TriviaError",
...
func omniCtx(arg any, client *sdk.TriviaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TriviaSDK); is {
			client = live
