# Trivia TypeScript SDK



The TypeScript SDK for the Trivia API — a type-safe, entity-oriented client with full async/await support.
...
```ts
const apis = await client.Api().list({ amount: 1 })
...
| `tester(testopts?, sdkopts?)` | `TriviaSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `TriviaSDK.test(testopts?, sdkopts?)` | `TriviaSDK` | Create a test-mode client. |
...
| `client` | `client(): TriviaSDK` | Return the parent SDK client. |
