...
//     (\`new TriviaSDK(...)\` / \`TriviaSDK.test(...)\` become
//     \`TriviaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"api":{"test01":{"id":"test01"}},"api_category":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TriviaSDK(...)\` and any
// \`TriviaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TriviaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TriviaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
