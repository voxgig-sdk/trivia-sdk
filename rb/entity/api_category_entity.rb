...
class ApiCategoryEntity
  ...
  def make
    opts = @_entopts.dup
    ApiCategoryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TriviaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ApiCategory, Hash] the current ApiCategory data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TriviaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ApiCategory fields)
  def match_get
  ...
  #
  # @param reqmatch [ApiCategoryListMatch, Hash, nil] match filter (any subset of
  #   ApiCategory fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ApiCategory>, Array] the matching ApiCategory items; raises TriviaError on failure
