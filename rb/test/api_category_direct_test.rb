...
class ApiCategoryDirectTest < Minitest::Test
  def test_direct_list_api_category
    setup = api_category_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-api_category", setup[:live] ? "live" : "unit")
    if _should_skip
...
def api_category_direct_setup(mockres)
  ...
  if live
    ...
    client = TriviaSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
