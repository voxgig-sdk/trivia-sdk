...
boru-driven command-line client **and** interactive REPL for the Trivia
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/trivia-cli)
make build
...
./trivia-cli list api
./trivia-cli list api_category

# 5. Override the API base URL for a single call
TRIVIA_BASE=https://api.example.com ./trivia-cli list api
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/trivia-cli
   ```
...
Run with no arguments to open a REPL (prompt `trivia>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/trivia-cli`. |
...
(`./trivia-cli <expr>`) and interactively (the REPL), and expressions compose the
