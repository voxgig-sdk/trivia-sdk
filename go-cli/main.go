// Command Trivia-cli is a CLI + REPL for the Trivia SDK,
...
const entitiesHelp = "api api_category"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TRIVIA_APIKEY (api key), TRIVIA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TRIVIA_APIKEY carries the API key and
	// TRIVIA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TRIVIA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TRIVIA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTriviaSDK(opts)
