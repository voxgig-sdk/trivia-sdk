...
The Lua SDK for the Trivia API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Api(nil):list(nil, nil)
...
local result, err = client:Trivia(nil):load(
  { id = "test01" }, nil
)
