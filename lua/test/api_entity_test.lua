...
describe("ApiEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Api(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Api(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRIVIA_TEST_API_ENTID JSON to run live")
      return
    end
    ...
    local api_ref01_match = {}

    local api_ref01_list_result, err = api_ref01_ent:list(api_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(api_ref01_list_result)
...
function api_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TRIVIA_TEST_API_ENTID")
