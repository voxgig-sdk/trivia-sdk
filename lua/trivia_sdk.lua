...
TriviaSDK._make_feature = _make_feature


function TriviaSDK.new(options)
  local self = setmetatable({}, TriviaSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TriviaSDK:options_map()
...
function TriviaSDK:get_utility()
  return Utility.copy(self._utility)
end


function TriviaSDK:get_root_ctx()
  return self._rootctx
end


function TriviaSDK:prepare(fetchargs)
...
function TriviaSDK:direct(fetchargs)
...
-- by passing it.
function TriviaSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TriviaSDK:graphql(query, variables, ctrl)
...
-- Idiomatic facade: client:ApiCategory():list() / client:ApiCategory():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function TriviaSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TriviaSDK.new(sdkopts)
  sdk.mode = "test"
