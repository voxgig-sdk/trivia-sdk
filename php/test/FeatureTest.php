...
class FtCtrl extends TriviaControl
{
...
class FtClient
{
    ...
    private TriviaUtility $utility;

    public function __construct(TriviaUtility $utility, array $options)
    {
    ...
    public function get_utility(): TriviaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TriviaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TriviaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TriviaConfig::make_feature($name);
    ...
    private static function buildUrl(TriviaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TriviaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TriviaResponse($fetched);
        }
    ...
    private function fail(TriviaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TriviaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
