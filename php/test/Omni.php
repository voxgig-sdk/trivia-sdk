...
//    only - a typed TriviaContext there would make every ctx
...
final class TriviaOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TriviaContext($clean, null);
    ...
    public static function err_from_map($m): ?TriviaError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TriviaError(is_string($code) ? $code : '', $msg);
    }
