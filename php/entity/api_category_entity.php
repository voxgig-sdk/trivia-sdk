...
class ApiCategoryEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ApiCategoryEntity($this->_client, $opts);
    }

    /**
     * @param ApiCategory|array $args ApiCategory data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TriviaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ApiCategory|array The current ApiCategory data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ApiCategory fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TriviaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ApiCategory fields).
     */
    ...
    /**
     * List ApiCategory items matching the given filter.
     *
     * @param ApiCategoryListMatch|array|null $reqmatch Match filter (any subset
     *   of ApiCategory fields) as an assoc-array; ApiCategoryListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ApiCategory[]|array A list of ApiCategory items as assoc-arrays at
     *   the SDK boundary; throws TriviaError on failure (item-5 convention).
     */
