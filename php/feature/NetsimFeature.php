...
class TriviaNetsimFeature extends TriviaBaseFeature
{
    ...
    private function _simulate(TriviaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TriviaContext $ctx, array $applied): void
    {
        $client = $this->client;
