...
class TriviaSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TriviaHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TriviaHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TriviaFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TriviaFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TriviaUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TriviaHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TriviaHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TriviaHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TriviaError($op . "_allow",
                "TriviaSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TriviaHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TriviaError("graphql_error",
                "TriviaSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_api_category = null;

    // Canonical facade: $client->ApiCategory()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->api_category()
    // resolves here too.
    public function ApiCategory($data = null)
    {
        require_once __DIR__ . '/entity/api_category_entity.php';
        if ($data === null) {
            if ($this->_api_category === null) {
                $this->_api_category = new ApiCategoryEntity($this, null);
            }
            return $this->_api_category;
        }
        return new ApiCategoryEntity($this, $data);
    }
