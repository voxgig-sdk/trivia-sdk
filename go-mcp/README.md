...
[MCP](https://modelcontextprotocol.io) server exposing the Trivia SDK as
two agent tools — `trivia_list` and `trivia_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/trivia-mcp)
make build

# 2. Provide credentials via the environment
export TRIVIA_APIKEY=sk_live_xxx
...
```jsonc
// trivia_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// trivia_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/trivia-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user trivia \
     -- "$PWD"/dist/*/trivia-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `trivia_list` and `trivia_load` tools now appear
   in new sessions. Ask the agent to *"list api using trivia"*
   and it calls `trivia_list` with `{"entity":"api"}`.
...
```sh
export TRIVIA_APIKEY=sk_live_xxx            # API key
export TRIVIA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `trivia_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `trivia_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TRIVIA_APIKEY` | API key sent with every request. |
| `TRIVIA_BASE` | Optional override of the API base URL. |
